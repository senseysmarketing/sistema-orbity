  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("task_statuses").select("*").eq("agency_id", currentAgency?.id).eq("is_default", false).order("order_position");
        ...
        <div className="space-y-2">
          {allStatuses.map((status: any) => <div key={status.id} className="flex items-center justify-between p-3 border rounded-lg">
              ...
              <div className="flex items-center gap-2">
                {!status.is_default && <>
                    <Switch checked={status.is_active} onCheckedChange={checked => toggleMutation.mutate({
                id: status.id,
                isActive: checked
              })} />
