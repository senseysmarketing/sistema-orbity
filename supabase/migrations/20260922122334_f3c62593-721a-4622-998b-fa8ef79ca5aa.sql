...
BEGIN
  ...
  IF NOT public.user_belongs_to_agency(v_agency_id)
     OR NOT public.is_agency_admin(v_agency_id) THEN
  ...
  IF EXISTS (
    SELECT 1
    FROM unnest(
      coalesce(p_cancel_payment_ids, '{}'::uuid[])
      || coalesce(p_preserve_payment_ids, '{}'::uuid[])
    ) AS requested(id)
  ...
  IF coalesce(p_cancel_payment_ids, '{}'::uuid[])
     && coalesce(p_preserve_payment_ids, '{}'::uuid[]) THEN
